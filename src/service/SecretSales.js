...
export async function processRequestSecret() {
  try {
    const marketplaceId = "aa330c92-55a8-4c8f-929c-a7c057db82fb";

    const datefin = currentDate();
    const dateinicio = currentDateMenos(3);

    const myHeaders = {
      Authorization: "9fb2eaca-9efd-412e-b83e-0f32cdcf8b95",
      Accept: "application/json",
    };

    let endpoint = `https://secretsales-prod.mirakl.net/api/orders?shop_id=<SID>&paginate=false&start_date=${dateinicio}T00:00:00.000000Z&end_date=${datefin}T23:59:00.000000Z&order_state_codes=WAITING_ACCEPTANCE,SHIPPING`;
    let shopsId = [3186, 3187, 3188];
    let orders = [];
    for (let i = 0; i < shopsId.length; i++) {
      const oResulShopPremiunOutlet = await axios({
        url: endpoint.replace("<SID>", shopsId[i]),
        method: "GET",
        headers: myHeaders,
      });
...
async function processOrder(order, marketplaceId) {
  ...
  if (filteredDetails.length > 0) {
    ...
    try {
      ...
      console.log("result: ", result.rows[0].save_sales_order_full);
